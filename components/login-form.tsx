...
function LoginFormContent({ className, ...props }: React.ComponentProps<"div">) {
  ...
  // Add "not logged in" message for better UX
  if (session) {
    return (
      ...
      <div className={cn("w-full max-w-md px-4", className)} {...props}>
        ...
        {searchParams.get('callbackUrl') && (
          <div className="mb-4 bg-red-500/20 border border-red-500/30 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <p className="text-red-300 font-medium">You are not logged in</p>
            </div>
            <p className="text-red-200/80 text-sm mt-2">Please sign in to access this page</p>
          </div>
        )}
        ...
        <Card className="glass-card border-white/20">
          <CardHeader className="text-center">
            ...
            <CardDescription className="subtle-text">Sign in to your account to continue</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Button
              variant="outline"
              className="w-full flex items-center justify-center glass border-white/20 text-white hover:bg-white/10 bg-transparent"
              type="button"
