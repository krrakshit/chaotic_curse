...
const Squares: React.FC<SquaresProps> = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // Set line properties for smoother rendering
      ctx.lineWidth = 0.5;
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';
      ...
      for (let x = startX; x < rect.width + squareSize; x += squareSize) {
        for (let y = startY; y < rect.height + squareSize; y += squareSize) {
          ...
          // Use more subtle border color with opacity
          ctx.strokeStyle = typeof borderColor === 'string' 
            ? borderColor.replace(')', ', 0.3)').replace('rgb', 'rgba')
            : borderColor;
          ctx.strokeRect(squareX, squareY, squareSize, squareSize);
        }
      }

      // Add a subtle gradient overlay for better visual effect
