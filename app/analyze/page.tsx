...
const LANGUAGES = [
  'python', 'javascript', 'java', 'c++', 'c', 'typescript', 'go', 'ruby', 'swift', 'kotlin'
];
...
export default function TimeComplexityAnalyzer() {
  ...
  return (
    <div className="glass-card rounded-2xl p-6 max-w-2xl mx-auto my-12">
      <h2 className="text-2xl font-bold mb-4 text-white">Time Complexity Analyzer</h2>
        ...
        {result && (
          <div className="mt-6">
            <div className="mb-2">
              ...
              <span className="text-blue-400 font-mono">{result.complexity}</span>
            </div>
            <div className="mb-2">
            ...
            <div className="mt-4">
              <span className="font-bold text-white">Operations Graph:</span>
              <div className="w-full h-48 mt-2 bg-white/5 rounded-lg flex items-end gap-2 p-4 border border-white/10">
                {result.graphData.map(({ n, ops }) => (
                  <div key={n} className="flex flex-col items-center justify-end h-full">
                    <div
                      className="bg-blue-400 rounded w-6"
                      style={{
                        height: `${Math.max(ops, 1) * 8}px`,
                        minHeight: '8px',
                        transition: 'height 0.3s'
                      }}
                      title={`n=${n}, ops=${ops}`}
                    />
                    <span className="text-xs text-gray-300 mt-1">n={n}</span>
                  </div>
                ))}
              </div>
            </div>
